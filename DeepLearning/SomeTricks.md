...
+ 当模型遇到badcase的时候，简单粗暴地增加模型的batch size效果可能并不好。</br>badcase通常和项目背景强相关，这种情况下最好的方法是收集并进行badcase分析。可能会有使用该模型的人提供badcase，但这种效率比较低，取决于数据提供者的心情or紧急程度；也可以直接捞一大批模型使用场景的query并使用当前模型做检测，收集相应类别置信度比较低的case，然后挑选出来。
